...
async function helper(k) {
    ...
    return {delX, delY, amount0, amount1};

}

(async () =>{

    let {delX, delY, amount0, amount1} = await helper(1);
    let k = amount1/delY;
    console.log("k = "+k);
    let dustX = Math.abs(delX-amount0);
    let dustY = Math.abs(delY-amount1);
    if(dustX <= 30 && dustY <= 0.1) {
        console.log(k);
        return;
    }

    let lo=k,hi=1,count=1;
    while(hi-lo > 0.0001 && count<=1000){
        count++;
        k=(hi+lo)/2;
        console.log("Checking for k = "+k);
        ({delX, delY, amount0, amount1} = await helper(k));
        dustX = Math.abs(delX-amount0);
        dustY = Math.abs(delY-amount1);
        if(dustX <= 30 && dustY <= 0.1) break;

       if(dustX >= 30 || dustY <= 0.1) lo=k;
        else hi=k;
    }
